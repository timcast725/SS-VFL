...
# VFL arguments
parser.add_argument('--client', default=0, type=int,
                    help='the client ID (indicates which data to receive)')
...
def main():
    ...
    save_folder_terms = [
        f'mocom_mvcnn3{args.client}_{args.moco_m:g}',
    ]
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        images_view = images[args.client]

        images_view[0] = images_view[0].cuda(args.gpu, non_blocking=True)
        images_view[1] = images_view[1].cuda(args.gpu, non_blocking=True)
        print(images_view[0].shape)

        # compute losses
        moco_losses = model(im_q=images_view[0], im_k=images_view[1])
        total_loss = moco_losses.combine(contr_w=args.moco_contr_w, align_w=args.moco_align_w, unif_w=args.moco_unif_w)

        # record loss
        if args.index == 0:
            bs = images_view[0].shape[0]
