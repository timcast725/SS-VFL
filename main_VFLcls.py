...
def main_worker(index, args):
        ...
        if m != args.num_clients:
            ...
            if args.mode != "supervised":
                if os.path.isfile(args.pretrained):
                    # Change filename check to be specific for the client
                    filename_arr = args.pretrained.split('_')
                    filename_arr[1] = f"VFL{m}"
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            for m in range(args.num_clients+1):
                filename_arr = args.resume.split('/')
                filename_arr[-1] = f"client{m}_model.pth.tar"
                filename = '/'.join(filename_arr)
                print("=> loading checkpoint '{}'".format(filename))
                # Map model to be loaded to specified single gpu.
                checkpoint = torch.load(filename, map_location=torch.device('cuda', args.gpu))
                args.start_epoch = checkpoint['epoch']
                best_acc1 = checkpoint['best_acc1']
                if isinstance(best_acc1, torch.Tensor):
                    # best_acc1 may be from a checkpoint from a different GPU
                    best_acc1 = best_acc1.to(args.gpu)
                models[m].load_state_dict(checkpoint['state_dict'])
                optimizers[m].load_state_dict(checkpoint['optimizer'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (args.distributed and args.rank == 0) or (args.index == 0):
            ...
            for m in range(args.num_clients+1):
                save_filename = os.path.join(args.save_folder, f'client{m}_model.pth.tar')
                save_checkpoint({
                    'epoch': epoch + 1,
                    'arch': args.arch,
